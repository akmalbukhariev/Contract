    ...
    public class CreateHtmlPDF
    {
        ...
        {   
            ...
            for (int i = 0; i < JsonList.Count; i++)
            {
                ...
                {
                    strHtml += ContractTextEditor.CreateTitle($"{item.Title} {item.Description}");
                    for (int j = 0; j < item.Child.Count; j++)
                    {
                        strHtml += ContractTextEditor.CreateDescription($"{item.Child[j].Title}",$"{item.Child[j].Description}");
                    }
                }
            }
